...
ds_train = Dataset_CSV_3d(csv_file=csv_train, image_shape=image_shape)
loader_train = DataLoader(ds_train, batch_size=batch_size_train,
                          sampler=sampler, num_workers=4)
ds_valid = Dataset_CSV_3d(csv_file=csv_valid, image_shape=image_shape)
loader_valid = DataLoader(ds_valid, batch_size=batch_size_valid,
                          num_workers=4)
ds_test = Dataset_CSV_3d(csv_file=csv_test, image_shape=image_shape)
...
#region training
save_model_dir = f'/tmp2/2020_2_20/{data_version}'
train_times = 1
train_type = 'pre_trained'

# ['resnet34', 'resnet50', 'ModelsGenesis']
for train_time in range(train_times):
    for model_name in ['resnet50']:
        ...
        loss_class_weights = torch.FloatTensor(loss_class_weights).cuda()
